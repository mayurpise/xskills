...
REPORT_DIR_RE = re.compile(r"^XSECURITY-[0-9][0-9-]*\Z")
...
def header_comment(unit: Unit, base: str, report_ref: str) -> str:
    ...
    lines = [
        f"# xsecurity -- suggested patch for {unit['id']}: {unit['title']}",
...
def note_written(unit: Unit, stats: list[DiffStat] | None, check: str, report_ref: str) -> str:
    ...
    lines += [
        ...
        "```",
        "",
        "Or ask xsecurity to apply it, or to open a pull request for it.",
        "",
    ]
    return "\n".join(lines)
...
def index_markdown(units: list[Unit], base: str, report_dir_name: str, report_ref: str) -> str:
    ...
    lines += [
        ...
        (
            ...
            "if you want one applied, or turned into a pull request, ask xsecurity "
            "and it handles that as a separate request."
        ),
        "",
    ]
    return "\n".join(lines)
...
def resolve_report_dir(patches_dir: str) -> tuple[str, str]:
    ...
    if not REPORT_DIR_RE.match(report_dir_name):
        msg = (
            "patches dir must live inside an XSECURITY-<timestamp> report "
...
def refuse_reason(path: str) -> str | None:
    ...
    Only `<report>/.xsecurity-run/patch-<ts>/scratch-F<n>` holding its
...
def remove_patch_run(patch_dir: str) -> tuple[list[str], list[str]]:
    ...
    `<report>/.xsecurity-run/patch-<ts>` layout is deleted.
    """
    removed: list[str] = []
