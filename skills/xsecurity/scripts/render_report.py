...
Writes XSECURITY-RESULTS.jsonl (one finding per line, fields in a fixed
order) and the XSECURITY-REVISION-<tag>.json stamp, places the report
...
REVISION_PREFIX = "XSECURITY-REVISION-"
...
def render(run_dir: str, products_dir: str) -> tuple[list[Finding], VerificationSummary, str]:
    ...
    if not os.path.isfile(markdown_path):
        raise RenderError(
            "XSECURITY-RESULTS.md is missing. Write the human-readable "
    ...
    atomic_write(
        os.path.join(products_dir, "XSECURITY-RESULTS.jsonl"),
        "".join(jsonl_line(f) + "\n" for f in findings),
    )
    markdown_out = os.path.join(products_dir, "XSECURITY-RESULTS.md")
...
def main(argv: list[str]) -> int:
    ...
    print(
        f"wrote XSECURITY-RESULTS.jsonl ({len(findings)} finding"
