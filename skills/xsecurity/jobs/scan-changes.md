...
**Engine check — before creating anything.** The workflow is `xsecurity-scan`. Prefer `Workflow({ name: "xsecurity-scan", … })`. If that name is unknown, resolve a `scriptPath`: run `ls "$HOME"/.*/workflows/xsecurity-scan.js` as its own Bash call (a glob over the user's tool dirs). First existing file wins — if more than one matches, prefer a path under `.grok/`. If the name is unknown **and** no file exists, no scan engine is installed in this session — stop cleanly and say so in one line. Do not invent a substitute pipeline (the report directory created below then holds only its `.gitignore`; say so).
...
```
Workflow({ name: "xsecurity-scan",
           // or, when the name is unknown: scriptPath: "<absolute xsecurity-scan.js from the engine check>",
