...
Written by the Security Lead into the patch working ground (`<report dir>/.xsecurity-run/patch-<ts>/patches.json`). One object with a `units` array, one entry per selected finding:
