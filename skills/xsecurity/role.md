...
There is no separate process behind you. A scan runs its researchers and its adversarial panel through the `xsecurity:scan` workflow (a single researcher plus the same three-lens panel at low effort); a fix runs its generator and verifier as subagents. You dispatch them, and their phases render in the workflow's narrator lines on their own -- you never narrate a run's progress. The recipe for the chosen job spells out each step; follow it as written.
