...
| **Bounded diffs, stated coverage** | Lockfiles, generated code, vendored trees, and build output are named as excluded rather than read; when what remains is still too large, the highest-risk files go first and the report says which changed files went unreviewed. The same honesty audit mode has always had, now on every diff. |
...
**Adversarial validation as the trust mechanism.** Every bug, silent-failure, security, contract, concurrency, and CLAUDE.md candidate is re-checked with a refute-by-default pass that must state a concrete failure (which inputs → what wrong result) before the finding survives. This directly targets the plausible-but-wrong finding that erodes reviewer trust — the failure mode that makes teams ignore automated review.

**The validator is given the claim, not the argument.** It receives the file, the line, and the assertion — never the finder's reasoning or its confidence score — and has to re-derive the failure from the code in its own words. A check that inherits the case it is meant to break is not a check. Where the repository makes it cheap, the pass goes further and *runs* something: the project's typechecker over the changed files, or the one existing test covering the path. A finding that survives execution is the strongest evidence the skill can produce; one the compiler contradicts is dropped.

**Severity is impact; confidence is certainty.** Two axes, never collapsed into one number. Confidence gates and only gates — below 80 a finding is discarded, the sole exception being a CLAUDE.md rule quoted verbatim, where the quote is itself the evidence. Severity ranks what survives, by what it costs if real. Collapsing them is how a certain triviality gets filed as Critical while an uncertain catastrophe gets promoted on dread alone. The explicit false-positive list (pre-existing issues, linter-catchable nits, correct-but-suspicious code) names the exact categories to suppress — with one carve-out, since existing code the change makes newly reachable is shipping *because of* this change.

**What a diff removes is reviewed as closely as what it adds.** Every dimension is naturally phrased around new code, which leaves the most dangerous edit in any change set — a deleted bounds check, auth check, timeout, or test — matching no pass at all. A removal with nothing taking its place is held to the same bar as any other finding: state what now gets through that did not before.

**The costliest defects sit at the diff's edges.** A changed signature whose callers were not updated, semantics quietly altered under a stable name (units, timezone, inclusive bounds), a migration that cannot roll back, a config key deleted while deploy scripts still set it — none of these are visible inside the changed lines. One dimension is licensed to follow a contract one hop out of the diff and name the caller or deployment step that breaks.

**Tests the change touched are code under review, not proof.** A green suite means nothing if the assertions were weakened, the test was skipped alongside its subject, or a snapshot was regenerated to match new output. A change that edits its own failing test into agreement is the most common way a defect reaches main.
