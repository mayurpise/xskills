...
| `skills/xsecurity/SKILL.md` | `/xsecurity` skill — a team of agents run as security researchers: scan the codebase, scan changes (working tree, branch, PR diff, or one commit), or turn confirmed findings into targeted `.patch` files; every finding survives a three-voter adversarial panel and every patch a verifier plus a fresh red-team pass before it is written — nothing is applied, committed, or pushed |
